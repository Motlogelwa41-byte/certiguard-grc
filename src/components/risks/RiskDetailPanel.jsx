...
import { X, Shield, FileCheck, ChevronRight, Loader2, ExternalLink } from "lucide-react";
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-card border-l border-border z-50 flex flex-col shadow-2xl">
        ...
        <div className="flex items-start justify-between gap-3 px-5 py-4 border-b border-border shrink-0">
          <div className="flex-1 min-w-0">
            {risk.risk_id && (
              <p className="text-[10px] font-mono text-muted-foreground mb-0.5">{risk.risk_id}</p>
            )}
            <h2 className="text-base font-heading font-bold text-foreground leading-snug">{risk.title}</h2>
            <div className="flex flex-wrap items-center gap-2 mt-2">
              <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${SCORE_BG(score)}`}>
                Score {score}
              </span>
