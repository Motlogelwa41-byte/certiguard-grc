...
function generateExcelFile(template, sections, data) {
  const { controls, risks, evidence, audits, incidents, tasks, frameworks, vendors } = data;
  const rows = [];
  rows.push(["EthicalEdge GRC — Custom Report"]);
  rows.push([template.name]);
  ...
  for (const section of sections) {
    ...
    switch (section.type) {
      case "executive_summary":
        ...
        rows.push(["Frameworks tracked", frameworks.length]);
        rows.push(["Total controls", controls.length]);
        rows.push(["Passing controls", controls.filter((c) => c.status === "passing").length]);
        rows.push(["Failing controls", controls.filter((c) => c.status === "failing").length]);
        rows.push(["Open risks", risks.filter((r) => r.status === "open").length]);
        rows.push(["Pending tasks", tasks.filter((t) => t.status !== "completed").length]);
        rows.push(["Open incidents", incidents.filter((i) => i.status !== "closed").length]);
        break;
      case "framework_readiness":
        rows.push(["Framework", "Readiness Score"]);
        frameworks.forEach((fw) => rows.push([fw.name, `${Math.round(fw.readiness_score || 0)}%`]));
        break;
      case "control_status":
        rows.push(["Status", "Count", "Percentage"]);
        ["passing", "failing", "not_tested", "not_applicable"].forEach((s) => {
          const count = controls.filter((c) => c.status === s).length;
          const pct = controls.length > 0 ? Math.round((count / controls.length) * 100) : 0;
          rows.push([s.replace(/_/g, " "), count, `${pct}%`]);
        });
        break;
      case "risk_overview":
        rows.push(["Title", "Category", "Score", "Status", "Owner"]);
        risks.forEach((r) => rows.push([r.title, r.category, r.risk_score || 0, r.status, r.owner_name || "—"]));
        break;
      case "evidence_summary":
        rows.push(["Status", "Count"]);
        rows.push(["Approved", evidence.filter((e) => e.status === "approved").length]);
        rows.push(["Pending review", evidence.filter((e) => e.status === "pending_review").length]);
        rows.push(["Rejected", evidence.filter((e) => e.status === "rejected").length]);
        rows.push(["Total", evidence.length]);
        break;
      case "kpi_dashboard":
        rows.push(["KPI", "Value"]);
        const avgEff = controls.length > 0 ? Math.round(controls.reduce((s, c) => s + (c.effectiveness_score || 0), 0) / controls.length) : 0;
        rows.push(["Average control effectiveness", `${avgEff}%`]);
        rows.push(["Controls with evidence", controls.filter((c) => c.evidence_count > 0).length]);
        rows.push(["Automated controls", controls.filter((c) => c.automation_status === "automated").length]);
        break;
      case "audit_summary":
        rows.push(["Audit", "Status", "Result"]);
        audits.forEach((a) => rows.push([a.title, a.status, a.result || "Pending"]));
        break;
      case "incident_summary":
        rows.push(["Severity", "Title", "Status"]);
        incidents.filter((i) => i.status !== "closed").forEach((i) => rows.push([i.severity, i.title, i.status]));
        break;
      case "vendor_summary":
        rows.push(["Vendor", "Risk Level", "Status"]);
        vendors.forEach((v) => rows.push([v.name, v.risk_level || "N/A", v.status || "active"]));
        break;
      case "custom_table":
        rows.push(["Custom table — configure filters in the template editor."]);
        break;
    }
    rows.push([]);
  }
  ...
  const html = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="utf-8"></head><body><table>${rows.map((r) => `<tr>${r.map((c) => `<td>${String(c ?? "").replace(/</g, "&lt;")}</td>`).join("")}</tr>`).join("")}</table></body></html>`;
  const blob = new Blob([html], { type: "application/vnd.ms-excel;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `${template.name.replace(/\s+/g, "_")}_${new Date().toISOString().slice(0, 10)}.xls`;
...
export default function CustomReportBuilder() {
  ...
  const generateReport = async (template) => {
    ...
    try {
      ...
      const reportData = { controls, risks, evidence, audits, incidents, tasks, frameworks, vendors };

      if (template.format === "excel" || template.format === "both") {
        generateExcelFile(template, sections, reportData);
      }
      ...
      toast({ title: `Report generated (${template.format === "both" ? "PDF + Excel" : template.format.toUpperCase()})` });
    } catch (e) {
          ...
          {templates.map((t) => (
            <div key={t.id} className="bg-card rounded-xl border border-border p-5 flex flex-col">
              ...
              <Button onClick={() => generateReport(t)} disabled={generating === t.id} className="mt-auto w-full" variant="outline">
                {generating === t.id ? <><Loader2 className="w-4 h-4 mr-1.5 animate-spin" /> Generating...</> : <><FileDown className="w-4 h-4 mr-1.5" /> Generate {t.format === "both" ? "PDF + Excel" : (t.format || "pdf").toUpperCase()}</>}
              </Button>
            </div>
          ))}
        </div>
